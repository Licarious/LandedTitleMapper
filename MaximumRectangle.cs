namespace LandedTitleMapper
{
    public class MaximumRectangle
    {
        ...
        public static ((int, int), (int, int)) Center(HashSet<(int, int)> coordList, bool floodFill, bool useMaxSquare = false) {
            _ = (0, 0);
            _ = (0, 0);

            int minX = coordList.First().Item1;
            int maxX = coordList.First().Item1;
            int minY = coordList.First().Item2;
            int maxY = coordList.First().Item2;
            //find min and max x and y
            foreach ((int, int) coord in coordList) {
                if (coord.Item1 < minX) {
                    minX = coord.Item1;
                }
                if (coord.Item1 > maxX) {
            ...
            //fill matrix with 1s where there is a pixel in coordList
            foreach ((int, int) coord in coordList) {
                A[coord.Item1 - minX][coord.Item2 - minY] = 1;
            }
